...
def build_model(alphabet, input_dim, hidden_dim, layers, feature_types, feat_input_dim, feature_alphabet):
    ...
    # 2 * HIDDEN_DIM + 3 * INPUT_DIM, as it gets a concatenation of frnn, rrnn, previous output char, i, j
    decoder_rnn = LSTMBuilder(layers, 2 * hidden_dim + 3 * input_dim, hidden_dim,
                              model)

    # remove feats from decoder, add layer to convert feats dim to hidden dim
    # 2 * len(feature_types) * feat_input_dim, as it gets both source and target feature embeddings
    model.add_parameters("feats_layer", (hidden_dim, 2 * len(feature_types) * feat_input_dim))
    model.add_parameters("feats_bias", hidden_dim)
...
def train_model(model, encoder_frnn, encoder_rrnn, decoder_rnn, train_source_words, train_source_feat_dicts,
                train_target_words, train_target_feat_dicts, dev_source_words,
...
def one_word_loss(model, encoder_frnn, encoder_rrnn, decoder_rnn, source_word, source_feats, target_word,
    ...
    feats_layer = parameter(model["feats_layer"])
    feats_bias = parameter(model["feats_bias"])
    ...
    # the vector reperesenting the input features
    ...
    for i in xrange(input_seq_len):
        blstm_outputs.append(concatenate([frnn_outputs[i], rrnn_outputs[input_seq_len - i - 1]]))
    ...
    # i is input index (starts with 1 as we skip the feats input), j is output index
    i = 1
    j = 0
    ...
    aligned_source_word = aligned_source_word + END_WORD
    aligned_target_word = aligned_target_word + END_WORD

    # TODO: remove feats from decoder input - done
    # TODO: add feats vector as the first element in the input sequence - done
    # TODO: try adding as input to the decoder? try adding to both ends of input sequence?

    # continue normally
    # run through the alignments
    for index, (input_char, output_char) in enumerate(zip(aligned_source_word, aligned_target_word)):
        possible_outputs = []
        ...
        # if there is no prefix, step. i-1 as feats are not in the source word sequence
        if source_word[i - 1] == BEGIN_WORD and aligned_source_word[index] != ALIGN_SYMBOL:
            # perform rnn step
        ...
        # if it is a character to predict
        if aligned_target_word[index] != ALIGN_SYMBOL:
            decoder_input = concatenate([prev_output_vec,
                                         char_lookup[alphabet_index[str(i)]],
                                         char_lookup[alphabet_index[str(j)]],
                                         blstm_outputs[i]])

            # TODO: copy i action - maybe model as a single action?
            # check if it is a copy action, i - 1 as source_word does not contain the feats
            if source_word[i - 1] == aligned_target_word[j]:
                possible_outputs.append(str(i - 1))
                possible_outputs.append(source_word[i - 1])
            else:
                possible_outputs.append(aligned_target_word[index])
            ...
            # sum over all *correct* output possibilities (char/copy) and pick feedback output to be the one with
        ...
        # now check if it's time to progress on input - promote i by checking if there are more unexplored input chars
        # and the next input char is not a placeholder symbol
        if i - 1 < len(source_word) - 1 and aligned_source_word[index + 1] != ALIGN_SYMBOL:
            # perform rnn step
            # feedback, i, j, blstm[i], feats
            decoder_input = concatenate([prev_output_vec,
                                         char_lookup[alphabet_index[str(i)]],
...
def predict_inflection_template(model, encoder_frnn, encoder_rrnn, decoder_rnn, source_word, source_feats, 
    ...
    # BiLTSM outputs
    blstm_outputs = []
    source_word_char_vecs_len = len(input_seq)
