...
Usage:
  task1_ndst_twin_feedback.py [--cnn-mem MEM][--input=INPUT] [--hidden=HIDDEN]
...
def train_cluster_model(input_dim, hidden_dim, layers, cluster_index, cluster_type, train_lemmas, train_feat_dicts,
    ...
    # build model
    initial_model, encoder_frnn, encoder_rrnn, decoder_rnn, char_feedback_rnn, action_feedback_rnn = \
        build_model(alphabet, input_dim, hidden_dim, layers, feature_types, feat_input_dim, feature_alphabet)
    ...
    # train model
    trained_model, last_epoch = train_model(initial_model, encoder_frnn, encoder_rrnn, decoder_rnn, char_feedback_rnn,
                                            action_feedback_rnn, train_cluster_lemmas,
    ...
    predicted_templates = predict_templates(trained_model, decoder_rnn, encoder_frnn, encoder_rrnn, char_feedback_rnn,
                                            action_feedback_rnn, alphabet_index, inverse_alphabet_index,
                                            dev_cluster_lemmas, dev_cluster_feat_dicts, feat_index, feature_types)
...
def build_model(alphabet, input_dim, hidden_dim, layers, feature_types, feat_input_dim, feature_alphabet):
    ...
    # feedback rnn's
    char_feedback_rnn = LSTMBuilder(layers, input_dim, hidden_dim, model)
...
def train_model(model, encoder_frnn, encoder_rrnn, decoder_rnn, char_feedback_rnn, action_feedback_rnn, train_lemmas,
                train_feat_dicts, train_words, dev_lemmas, dev_feat_dicts, dev_words, alphabet_index,
                inverse_alphabet_index, epochs, optimization, results_file_path, morph_index, train_aligned_pairs,
                dev_aligned_pairs, feat_index, feature_types, plot):
    print 'training...'
    ...
    for e in xrange(epochs):
        ...
        for i, example in enumerate(train_set):
            ...
            loss = one_word_loss(model, encoder_frnn, encoder_rrnn, decoder_rnn, char_feedback_rnn, action_feedback_rnn,
                                 lemma, feats, word, alphabet_index, alignment, feat_index, feature_types)
        ...
        if EARLY_STOPPING:
            ...
            if len(dev_lemmas) > 0:
                ...
                for i in xrange(len(dev_lemmas)):
                    total_dev_loss += one_word_loss(model, encoder_frnn, encoder_rrnn, decoder_rnn, char_feedback_rnn,
                                                    action_feedback_rnn, dev_lemmas[i], dev_feat_dicts[i], dev_words[i],
...
def one_word_loss(model, encoder_frnn, encoder_rrnn, decoder_rnn, char_feedback_rnn, action_feedback_rnn, lemma, feats,
                  word, alphabet_index, aligned_pair, feat_index, feature_types):
    renew_cg()
    ...
    # TODO: twin feedback decoder - prev output vec is now a concatenation of actions feedback rnn and char feedback
    # TODO: rnn, concatenated and propagated through an MLP
    ...
    # i is input index pointer, j is output index pointer
    i = 0
    j = 0
    ...
    for index, (input_char, output_char) in enumerate(zip(aligned_lemma, aligned_word)):
        ...
        # first check if reached the end word symbol
        ...
        if aligned_word[index] != ALIGN_SYMBOL:
            ...
            # check if copy i action is suitable now as a correct prediction
            # TODO: maybe model as a single action?
        ...
        # finished)
        # TODO: this is a bit of a hard constraint, maybe we don't want to step according to the alignment every time
...
def predict_templates(model, decoder_rnn, encoder_frnn, encoder_rrnn, char_feedback_rnn, action_feedback_rnn,
                      alphabet_index, inverse_alphabet_index, lemmas, feats, feat_index, feature_types):
    predictions = {}
    for i, (lemma, feat_dict) in enumerate(zip(lemmas, feats)):
        predicted_template = predict_output_sequence(model, encoder_frnn, encoder_rrnn, decoder_rnn, char_feedback_rnn,
                                                     action_feedback_rnn, lemma, feat_dict, alphabet_index,
