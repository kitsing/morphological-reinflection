...
Usage:
  task2_ndst_sfp.py [--cnn-mem MEM][--input=INPUT] [--hidden=HIDDEN]
...
def one_word_loss(model, encoder_frnn, encoder_rrnn, decoder_rnn, source_word, source_feats, target_word,
    ...
    # run the decoder through the alignments
    for index, (input_char, output_char) in enumerate(zip(aligned_source_word, aligned_target_word)):
        ...
        if source_word[i] == '<' and aligned_source_word[index] != ALIGN_SYMBOL:
            # perform rnn step
            # feedback, i, j, blstm[i]
            s = s.add_input(decoder_input)
            decoder_rnn_output = s.output()
        ...
        if aligned_target_word[index] != ALIGN_SYMBOL:
            ...
            if source_word[i] == aligned_target_word[j]:
                possible_outputs.append(str(i))
                possible_outputs.append(source_word[i])
            else:
            ...
            # go through all correct output possibilities and pick feedback output to be the one with the highest
            # probability
        ...
        if i < len(source_word) - 1 and aligned_source_word[index + 1] != '~':
            # perform rnn step
            # feedback, i, j, blstm[i]
            decoder_input = concatenate([prev_output_vec,
                                         char_lookup[alphabet_index[str(i)]],
