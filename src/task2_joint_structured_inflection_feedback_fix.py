...
Usage:
  task2_joint_structured_inflection_feedback_fix.py [--cnn-mem MEM][--input=INPUT] [--hidden=HIDDEN] [--feat-input=FEAT]
...
def main(train_path, test_path, results_file_path, sigmorphon_root_dir, input_dim, hidden_dim, feat_input_dim, epochs,
    ...
    os.system('python task2_evaluate_best_joint_structured_models_feed_fix.py --cnn-mem 6096 --input={0} --hidden={1} --feat-input={2} \
...
def train_cluster_model(input_dim, hidden_dim, layers, cluster_index, cluster_type, train_source_words, train_source_feat_dicts,
    ...
    # train model
    trained_model = train_model(initial_model, encoder_frnn, encoder_rrnn, decoder_rnn, train_cluster_source_words,
...
def predict_inflection_template(model, encoder_frnn, encoder_rrnn, decoder_rnn, source_word, source_feats, 
    ...
    while i < MAX_PREDICTION_LEN:
        ...
        decoder_input = concatenate([encoded, prev_output_vec, source_word_input_char_vec,
